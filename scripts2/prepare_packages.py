...
This script reads prepare.yaml files from packages2/ and:
...
class PackagePreparer:
    ...
    def prepare_all_packages(self) -> bool:
        """Prepare all packages in packages2/."""
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        packages_dir = os.path.join(project_root, 'packages2')
        
        if not os.path.exists(packages_dir):
            print(f"Error: packages2 directory not found at {packages_dir}")
            return False
