...
```matlab
mip install --channel mip-org/dev geometry-processing-package
...
`test_geometry_processing_package.m` builds a triangulated grid mesh and exercises the I/O (OFF write/read round-trip), algebra (`face_area`, `laplace_beltrami` with rows summing to zero, `compute_edge` with boundary-edge detection), and topology (`dijkstra` shortest paths over the mesh graph) modules. The test is display-free (no `plot_mesh`).
