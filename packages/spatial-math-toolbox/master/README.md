...
`test_spatial_math_toolbox.m` exercises the core math: it verifies `rotx`/`roty`/`rotz` produce valid SO(3) matrices, round-trips RPY angles and SO(3) matrix exp/log (`trexp`/`trlog`), and checks the `SE3` and `UnitQuaternion` classes (composition, inverse, quaternion↔matrix). The test is display-free (no plotting).
